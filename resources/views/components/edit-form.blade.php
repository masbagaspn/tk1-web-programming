<form action="{{ route('update', $id) }}" method="post" enctype="multipart/form-data" class="space-y-8">
    {{ csrf_field() }}
    @method('PUT')
    <div class="flex flex-col gap-2">
        <label class="text-xs font-medium text-slate-500">Video Title</label>
        <input value="{{ $title }}" id='edit-title-input' name="title" class="bg-emerald-50 text-sm rounded-sm px-4 py-2 focus:ring-emerald-200 focus:outline-emerald-300 focus:outline-2" type='text'/>
    </div>
    <div class="flex flex-col gap-2">
    ...
    <div class="w-full flex justify-end gap-2">
        <button id="btn-cancel-edit-modal-{{ $id }}" type="button" class='btn-cancel-edit-modal text-xs bg-slate-100 px-4 py-2 rounded-md text-neutral-950 hover:bg-slate-200 transition'>Cancel</button>
